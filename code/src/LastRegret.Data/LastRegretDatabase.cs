...
public sealed class LastRegretDatabase : IDisposable
{
    ...
    {
        var raw = GetMeta(Events, SettingsKey);
        ...
        {
            // 设置损坏不能导致程序不可用
            SetMeta(Events, SettingsKey + ".corrupt." + DateTime.UtcNow.Ticks, raw);
            return new AppSettings();
        }
    }

    public void SaveSettings(AppSettings settings) =>
        SetMeta(Events, SettingsKey, JsonSerializer.Serialize(settings, AppSettings.JsonOpts));
}
